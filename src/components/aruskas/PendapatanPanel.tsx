...
import { fmt, ghk, kym, ymk, currentYM, isExpRutinActive } from "@/lib/helpers";
...
export default function PendapatanPanel({ onAddExtra, onAddRutin }: Props) {
  ...
  const [confirmTarget, setConfirmTarget] = useState<{ id: string; action: "delete" | "toggle" } | null>(null);
  ...
  const getRealizationBadge = (p: typeof pendapatanRutin[0]) => {
    const realized = isRealized(p.id);
    if (realized) return { label: "Terealisasi", cls: "bg-emerald-50 text-emerald-600" };
    if (p.realisasi === "manual") return { label: "Manual", cls: "bg-amber-50 text-amber-600" };
    if (p.tglBayar) {
      const today = new Date().getDate();
      if (isCurrentMonth && p.tglBayar > today) return { label: `Menunggu Tgl ${p.tglBayar}`, cls: "bg-blue-50 text-blue-500" };
    }
    return null;
  };

  const handleDeleteOrToggle = (id: string, action: "delete" | "toggle") => {
  ...
  const handleUndoCurrent = async () => {
    ...
    await unrealizeRoutine("income", confirmTarget.id, curBk);
    if (confirmTarget.action === "delete") doDelete(confirmTarget.id);
    else doToggle(confirmTarget.id);
  ...
  const confirmItem = confirmTarget ? pendapatanRutin.find((p) => p.id === confirmTarget.id) : null;
  ...
  return (
    <>
      <Card>
        ...
        {rutinItems.length === 0 ? (
          ...
          <div className="space-y-2 mb-4">
            {rutinItems.map((p) => {
              const badge = getRealizationBadge(p);
              return (
                <div key={p.id} className="flex items-center justify-between p-3 bg-emerald-50/80 border border-emerald-100/60 rounded-xl group">
                  <div className="flex items-center gap-2.5">
                    <button onClick={() => handleDeleteOrToggle(p.id, "toggle")} className="text-slate-400 hover:text-indigo-500 transition-colors flex-shrink-0">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button onClick={() => handleDeleteOrToggle(p.id, "delete")} className="p-1 rounded-lg opacity-0 group-hover:opacity-100 hover:bg-red-50 text-slate-400 hover:text-red-500 transition-all">
                      <Trash2 size={14} />
                    </button>
                  </div>
                </div>
      ...
      <ConfirmUndoModal
        ...
        title={confirmTarget?.action === "delete" ? "Hapus Pendapatan Rutin" : "Nonaktifkan Pendapatan Rutin"}
